...
export default function TenantList() {
  ...
  async function deactivateTenant(id) {
    ...
    if (tenant?.room_id) {
      await supabase.from('rooms').update({ is_occupied: false }).eq('id', Number(tenant.room_id))
    }
    fetchAll()
  }
              ...
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                  <div>
                    ...
                    <p style={{ color: 'gray', margin: '4px 0', fontSize: '14px' }}>
                      Room {room ? room.room_number : tenant.room_id} — {room ? room.room_type : ''} — {tenant.phone}
                    </p>
                    <p style={{ color: '#888', margin: '2px 0', fontSize: '12px' }}>
                      House Owner: {tenant.room_owner_name || tenant.house_owner || '—'}
                    </p>
                  </div>
                ...
                <div style={{ marginTop: '8px', fontSize: '14px', color: '#555' }}>
                  ...
                  {tenant.emergency_contact_name && (
                    <p style={{ margin: '2px 0', color: '#c00' }}>🚨 Emergency: {tenant.emergency_contact_name} ({tenant.emergency_contact_relation}) — {tenant.emergency_contact_phone}</p>
                  )}
                  {tenant.office_name && (
                    <p style={{ margin: '2px 0', color: '#0070f3' }}>🏢 Office: {tenant.office_name} — {tenant.office_contact_person} — {tenant.office_contact_phone}</p>
                  )}
                </div>
              </>
            )}
